...
export class HUD implements System {
  ...
  /** end of the sweep in mph, derived from the player's real top speed */
  private speedMax = 95;
  ...
  init(ctx: Ctx) {
    ...
    this.speedMax = dialMax(BASE_TOP_SPEED * topMul * BOOST_PEAK * HEADROOM * 2.2369362920544);
    this.needle.snap(0);
  }
  ...
  lateUpdate(ctx: Ctx, dt: number) {
    ...
    // --- speedometer -------------------------------------------------------
    const mph = Math.abs(player.forwardSpeed) * 2.2369362920544;
    const frac = clamp(mph / this.speedMax, 0, 1);
    this.needle.target = frac;
    this.needle.step(dt);
    setPair(this.speedNum, String(Math.round(mph)));

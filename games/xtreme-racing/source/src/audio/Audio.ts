...
export class Audio implements System {
  ...
  setMuted(musicMuted: boolean, sfxMuted: boolean) {
    this.musicMuted = musicMuted;
    this.sfxMuted = sfxMuted;
    const s = this.synth;
    if (!s) return;
    const now = s.ctx.currentTime;
    s.music.gain.setTargetAtTime(musicMuted ? 0.0001 : 0.38, now, 0.04);
    s.sfx.gain.setTargetAtTime(sfxMuted ? 0.0001 : 0.9, now, 0.04);
    s.engine.gain.setTargetAtTime(sfxMuted ? 0.0001 : 0.62, now, 0.04);
    s.lead.gain.setTargetAtTime(sfxMuted ? 0.0001 : 1, now, 0.04);
  }
  ...
  private build(ac: BaseAudioContext | null, vol: number): boolean {
    try {
      ...
      this.music.start();
      this.setMuted(this.musicMuted, this.sfxMuted);
      return true;
    } catch (err) {

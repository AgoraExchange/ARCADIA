...
const ARCADIA_BRIDGE_SOURCE = 'arcadia-xtreme-racing';
const ARCADIA_PARENT_SOURCE = 'arcadia-parent';
let bridgeFinishReported = false;

function postArcadia(type: string, detail: Record<string, unknown> = {}) {
  if (window.parent === window) return;
  window.parent.postMessage({ source: ARCADIA_BRIDGE_SOURCE, type, ...detail }, location.origin);
}

function postArcadiaState() {
  postArcadia('state', {
...
ctx.bus.on((event) => {
  if (event.type === 'countdown' && event.n === 3) {
    bridgeFinishReported = false;
    postArcadiaState();
  }
  ...
  postArcadia('finish', {
    ...
    bestLap: Number.isFinite(race.bestLap) ? race.bestLap : 0,
    lapTimes: [...race.lapTimes],
    racer: race.player.stats.name,
  });
});
...
function frame(now: number) {
  ...
  if (ctx.frame === 8) {
    ...
    dismissBootScreen();
    postArcadia('ready', { title: 'XTREME RACING' });
    postArcadiaState();
  }
}
